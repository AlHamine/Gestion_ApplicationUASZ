...
export default function AjouterEnseignement() {
  ...
  const addEnseignement = (enseignementSave) => {
    fetch(SERVER_URL + 'maquette/enseignement', {
      ...
      .then((response) => {
        if (response.ok) {
          alert('Enseignement ajoutée avec succès')
          backward()
          return response.json()
        } else {
      ...
      .then((data) => {
        ...
        console.log(data)
        //       const do= { libelle: data.libelle,
        // description: data.description,
        // // "idModule": 3,
        // // "module": "Mathématiques discretes 1",
        // // "cm": 0,
        // // "tp": 0,
        // // "td": 0,
        // // "credit_ue": 0,
        ...
        // // "nbreHeure": 5,
        // // "idClasse": 4,
        // // "classe": "Licence 2 MI Semestre 4",
        // // "idGroupe": 2,
        // // "groupe": "Groupe2",
        ...
        if (SERVER_URL == 'http://localhost:8080/') {
          fetch(SERVER_URL + 'repartition/maquette/enseignement', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(data),
          }).then((response2) => {
            if (response2.ok) alert('Enseignement ajoutée avec succès Cote Slave')
            backward()
          })
        }
      })
